...
function initializeDashboard() {
    ...
    // Solde courant et état depuis le backend
    fetch('api/compte/solde')
      .then(r => r.json())
      .then(data => {
        if (data && data.solde != null) {
          const soldeCourant = Number(data.solde);
          document.getElementById('solde-courant').textContent = `${soldeCourant.toFixed(2)} €`;
          // Recalcul du total global avec le solde dépôt simulé pour l'instant
          const totalGlobal = soldeCourant + mockData.accounts.depot.solde;
          document.getElementById('total-global').textContent = `${totalGlobal.toFixed(2)} €`;
        }
      })
      .catch(() => {});

    fetch('api/compte/etat')
      .then(r => r.json())
      .then(data => {
        if (data && data.etat) {
          const label = document.querySelector('#solde-courant').closest('.dashboard-card')
            .querySelector('.card-label');
          label.textContent = `État: ${data.etat}`;
        }
      })
      .catch(() => {});
    ...
    // Le total global est recalculé après récupération du solde courant
