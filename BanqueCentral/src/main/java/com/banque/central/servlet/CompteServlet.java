        ...
        try {
            ...
            if ("/solde".equals(pathInfo)) {
                ...
                if (compte != null) {
                    ...
                    // Récupérer le paramètre de devise
                    String currency = request.getParameter("currency");

                    if (currency != null && !currency.trim().isEmpty() && !"MGA".equals(currency)) {
                        // Utiliser la nouvelle méthode avec conversion
                        Method getWithCurrency = compteBean.getClass().getMethod(
                                "getTransactionsWithCurrency", Integer.class, String.class);
                        Object transactions = getWithCurrency.invoke(compteBean, compteId, currency);
                        out.print(gson.toJson(transactions));
                    } else {
                        // Transactions normales sans conversion
                        Method getTransactions = compteBean.getClass().getMethod("getTransactions", Integer.class);
                        Object transactions = getTransactions.invoke(compteBean, compteId);
                        out.print(gson.toJson(transactions));
                    }
                }
            }
            ...
            if (compte != null) {
                ...
                if ("depot".equals(action)) {
                    ...
                    if (montantStr == null) {
                        ...
                        out.print("{\"error\":\"Montant manquant\"}");
                        return;
                    }
                    ...
                    try {
                        ...
                        response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
                        out.print("{\"error\":\"Montant invalide\"}");
                        return;
                    }
                    ...
                    if (dateStr != null && !dateStr.isEmpty()) {
                        try {
                            opDate = new java.text.SimpleDateFormat("yyyy-MM-dd").parse(dateStr);
                    ...
                    boolean success;
                    if (opDate != null) {
                        Method m = compteBean.getClass().getMethod("depotAvecDate", Integer.class, BigDecimal.class,
                                String.class, java.util.Date.class);
                        success = (Boolean) m.invoke(compteBean, compteId, montant, mode, opDate);
                    } else {
                        Method m = compteBean.getClass().getMethod("depot", Integer.class, BigDecimal.class,
                                String.class);
                    ...
                    boolean success;
                    if (opDate != null) {
                        Method m = compteBean.getClass().getMethod("retraitAvecDate", Integer.class, BigDecimal.class,
                                String.class, java.util.Date.class);
                        success = (Boolean) m.invoke(compteBean, compteId, montant, mode, opDate);
                    } else {
                        Method m = compteBean.getClass().getMethod("retrait", Integer.class, BigDecimal.class,
                                String.class);
                        success = (Boolean) m.invoke(compteBean, compteId, montant, mode);
                    }
